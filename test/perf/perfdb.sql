...
AS $function$
  SELECT (CASE WHEN random() < _null_prob
               THEN NULL
...
AS $function$
  SELECT (CASE WHEN random() < _null_prob THEN NULL ELSE (random() < _true_prob) END);
$function$ LANGUAGE SQL VOLATILE;
...
AS $function$
  SELECT (CASE WHEN random() < _null_prob
               THEN NULL
               ELSE (SELECT string_agg(chr(rand_int(ascii('a'), ascii('z'))), '' ORDER BY i)
...
AS $function$
  SELECT (CASE WHEN random() < _null_prob THEN NULL ELSE rand_words(_len_from, _len_to) END);
$function$ LANGUAGE SQL VOLATILE;
...
BEGIN
  ...
  FOR i IN 1..10000 LOOP
    ...
    IF kind = 5 THEN
      INSERT INTO item_param_value (item_id, param_id, value)
        VALUES (i, 4, to_jsonb(round(rand_int(50, 100) / 100.0, 1))),
